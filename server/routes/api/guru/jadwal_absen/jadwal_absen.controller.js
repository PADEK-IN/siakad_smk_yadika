...
export const create = async (req, res) => {
  try {
    const { tanggal, waktu_buka, waktu_tutup, id_jadwal_pelajaran } = req.body;
    const validIdJadwal = checkValidId(id_jadwal_pelajaran);
    if (!validIdJadwal) return responses.res400('ID jadwal tidak valid', res);

    await Jadwal_Absen.create({
      tanggal,
      waktu_buka,
      waktu_tutup,
...
export const update = async (req, res) => {
  try {
    ...
    if (!validIdMaPel)
      return responses.res400('ID mata pelajaran tidak valid', res);
    ...
    if (!respons[0])
      return responses.res400('Maaf, jadwal absen tidak ditemukan', res);
