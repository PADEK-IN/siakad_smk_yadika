...
export const create = async (req, res) => {
  try {
    const idMurid = 1;
    const {id} = req.params;
    const idSpp = checkValidId(id);
    if(!idSpp) return responses.res400("ID SPP tidak valid", res);
    ...
    const dataTransaksi = await Transaksi.findOne({
        where: {id_spp:idSpp, id_murid:idMurid},
        raw: true
    });
    ...
    if(dataTransaksi) {
        ...
        const date = new Date();
        await Transaksi.create({id_spp:idSpp, id_murid:idMurid, tanggal_bayar: date, bukti_bayar: foto})
    }
